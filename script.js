...
document.addEventListener("click", e => {
  if (
    e.target.closest(".qc-app") ||
    e.target.closest(".app-icon") ||
    e.target.closest(".music-icon")
  ) {
    click.currentTime = 0;
    click.play().catch(()=>{});
  }
...
apps.forEach(a => {
  ...
  el.innerHTML = `
    <div class="app-icon-body" style="background:${a.c}">
      ${a.i}
    </div>
  ...
  s.className = "qc-app";
  s.innerHTML = `
    <div class="qc-app-icon" style="background:${a.c}">
      ${a.i}
    </div>
    ${a.n}
  `;
  s.onclick = () => window.open(a.f, "_blank"); // ✅ NEW TAB
  qcApps.appendChild(s);
});

console.log("✅ Apps now open in new tab");